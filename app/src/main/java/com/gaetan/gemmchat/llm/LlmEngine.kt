...
class LlmEngine(
    ...
    /**
     * Démarre une nouvelle conversation sans recharger le modèle : on recrée
     * simplement l'objet [Conversation] depuis l'[Engine] déjà initialisé.
     * No-op si le moteur n'est pas prêt.
     */
    suspend fun startNewConversation() = mutex.withLock {
        val activeEngine = engine ?: return@withLock
        conversation?.close()
        conversation = activeEngine.createConversation(buildConversationConfig())
    }
    ...
    companion object {
        ...
        init {
            // Charge le runtime LiteRT en amont pour exposer ses symboles (ex.
            // LiteRtCreateEnvironment) au plugin de sampling GPU chargé par dlopen.
            runCatching { System.loadLibrary("LiteRt") }
                .onFailure { Log.w(TAG, "Préchargement libLiteRt impossible", it) }
        }
    }
}